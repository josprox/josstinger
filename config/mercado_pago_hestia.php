...
if(isset($_POST['renovar'])){
    ...
    $preference_id = mercado_pago($rows['nombre'],$meses,$precio,"USD","hestia/public/users/renovar?token=$token_new&usr=$iduser&prdct=$servicio&mut=$meses","hestia/public/users/falla?usr=$iduser","hestia/public/users/pendiente");
...
if(isset($_POST['select'])){
    ...
    $preference_id = mercado_pago($rows['nombre'],$meses,$precio,"USD","hestia/public/users/hestia_config?token=$token&usr=$iduser&prdct=$servicio&mut=$meses","hestia/public/users/falla?usr=$iduser","hestia/public/users/pendiente");
    foreach(arreglo_consulta("SELECT nombre,descripcion FROM servicios WHERE id = $servicio") as $row){
        ?>
        <div class="block-7">
            <div class="text-center">
